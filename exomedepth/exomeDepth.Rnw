...
correlationWarning<-function(ref,limit.maxcor=0.98,limit.refcor=0.97,limit.refcount=3) {
...
plotRefsampleStats<-function() {
  bamstats.cor<-t(as.data.frame(do.call(rbind, lapply(bamstats, function(x) x[,'correlations']))))
  bamstats.phi<-t(as.data.frame(do.call(rbind, lapply(bamstats, function(x) x[,'phi']))))
  highlight<-ifelse(names(bamstats)==testsample,'hotpink','grey')
  cat("\\begin{multicols}{2}")
  boxplot(bamstats.cor, col=highlight)
  title(main="Correlation")
  #cat("\n\\columnbreak")
  boxplot(bamstats.phi, col=highlight)
  title(main="Dispersion")
  cat("\\end{multicols}")
  #cat("\\begin{quote}")
  #cat("Excessive dispersion or a lack of overall correlation can indicate a failed sample")
  #cat("\\end{quote}")
}
@
...
lowCoverageExons()
cat("\\subsubsection*{Sample comparison}")
plotRefsampleStats()

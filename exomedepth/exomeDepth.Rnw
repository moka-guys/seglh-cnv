...
if (any(qc$status!="PASS") || nrow(coverage.table)>0 || (!is.na(sexMismatch) && sexMismatch)) {
  cat("\\\\")
  qcWarning()
}
# CNV results
...
if (suppressWarnings(is.na(results[[testsample]]))) {
  ...
  if (!is.object(results[[testsample]])) {
    ...
    if (!hasAnnotation) cnvs[,'annotation']<-""
    if (!!nrow(cnvs)) {
      cnvtable<-cnvs[which(cnvs$BF>0),c('id','BF','reads.expected','reads.ratio','type','genes','confidence','annotation')]
      colnames(cnvtable)<-c('id','BF¹','expected','ratio','type','genes','confidence²',ifelse(hasAnnotation,'annotation',''))

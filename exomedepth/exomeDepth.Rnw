...
(\Sexpr{sanitize(length(coveredexons@ranges))} exons) \\
...
plotRefsampleStats<-function() {
  ...
  ymin<-min(keep.finite(c(limits$coeffvar,unlist(cvz.bands),batch.cv)))
  ymax<-max(keep.finite(c(limits$coeffvar,unlist(cvz.bands),batch.cv)))
...
failedQC<-any(qc$status!="PASS")
failedCoverage<-nrow(coverage.table)>0
failedSexmatch<-!any(is.na(sexMismatch)) && any(sexMismatch)

...
lowCoverageExons<-function(pagerows=55) {
  if (nrow(coverage.table)>0) {
    cat(paste0("Low coverage could reduce CNV detection sensitivity. Regions with median below ",limit.coverage,"x coverage:\\\\"))
    cat("\\begin{scriptsize}")
    ...
    for (i in 1:subtables) {
      ...
      print(kbl(coverage.table[startrow:endrow,], row.names=FALSE, format='latex', booktabs=TRUE))
    }
    cat("\\end{scriptsize}")
  } else {
    cat(paste0("Median coverage exceeds ",limit.coverage,"x in all target regions."))
  }
}
CNVtable<-function(cnvtable) {
  footnotes=c("The Bayes Factor (BF) is a measure of strength of evidence of the alternate hypothesis (copy number change)",
  ...
  cat("\\begin{scriptsize}")
  print(
    kbl(cnvtable, row.names=FALSE, format='latex', booktabs=TRUE) %>%
